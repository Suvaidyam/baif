...
function handleFrequencyField(dt, dialog) {
    let frequency = dialog?.fields_dict?.frequency?.value;
    if (!frequency) {
        return;
    }
    let start_date = dialog?.fields_dict?.start_date?.value || cur_frm?.doc?.start_date;
    let end_date = dialog?.fields_dict?.end_date?.value || cur_frm?.doc?.end_date;
    let result = generateTimeframes(start_date, end_date, frequency);
    if (result?.timeframes?.length) {
        let data = result?.timeframes.map((tf) => {
            return {
                timespan: tf?.period,
                year: tf?.year,
                ...
                end_date: tf?.endDate,
                month: tf?.month_no,
                quarter: tf?.quarter_no,
                half: tf?.half_no,
            }
        })
        dt.form_dialog.set_df_property('planning_table', 'data', data);
    }
}
